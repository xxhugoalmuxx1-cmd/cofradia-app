...
export default function Lottery() {
  ...
  async function loadHistory() {
    const { data } = await api.get("/lottery/sales", {
      params: historyFilter === "all" ? {} : { unitType: historyFilter },
    });
    setHistory(data);
  }
  ...
  function itemFormFor(campaignId: string): ItemForm {
    return itemForms[campaignId] || { number: "", unitType: "decimo", price: "", donationAmount: "", unitsReceived: "" };
  }
  ...
  async function createItem(campaignId: string, e: React.FormEvent) {
    ...
    try {
      ...
      setItemForms((prev) => ({ ...prev, [campaignId]: { number: "", unitType: "decimo", price: "", donationAmount: "", unitsReceived: "" } }));
      load();
    } catch (err: any) {
  ...
  function saleFormFor(itemId: string): SaleForm {
    return saleForms[itemId] || { unitsSold: "1", amountPaid: "", establishmentName: "" };
  }
  ...
  async function registerSale(item: any, e: React.FormEvent) {
    ...
    try {
      ...
      setSaleForms((prev) => ({ ...prev, [item.id]: { unitsSold: "1", amountPaid: "", establishmentName: "" } }));
      load();
      if (showHistory) loadHistory();
    } catch (err: any) {
      ...
      {showHistory && (
        <div className="bg-white rounded-xl shadow-sm p-4 space-y-3">
          ...
          <ul className="divide-y">
            {history.map((s: any) => (
              <li key={s.id} className="py-2 text-sm">
                <div className="flex justify-between flex-wrap gap-1">
                  <span>
                    Número {s.lotteryItem?.number} ({s.lotteryItem?.unitType === "sabana" ? "sábana" : "décimo"}) — {s.unitsSold} uds
                    {s.establishmentName && <span className="text-gray-500"> · vendido en {s.establishmentName}</span>}
                    {s.member && <span className="text-gray-500"> · {s.member.firstName} {s.member.lastName}</span>}
                  </span>
            ...
            {history.length === 0 && <p className="text-sm text-gray-400">Sin ventas registradas todavía.</p>}
          </ul>
        </div>
      )}
      ...
      {campaigns.map((c) => (
        <div key={c.id} className="bg-white rounded-xl shadow-sm p-4 space-y-4">
          ...
          <ul className="divide-y">
            {c.items?.map((item: any) => (
              <li key={item.id} className="py-3 text-sm space-y-2">
                <div className="flex justify-between items-center flex-wrap gap-1">
                  <span className="font-medium">
                    ...
                    <span className="text-xs font-normal text-gray-500">
                      ({item.unitType === "sabana" ? "sábana" : "décimo"})
                    </span>{" "}
                ...
                <form onSubmit={(e) => registerSale(item, e)} className="flex flex-wrap gap-2">
                  <input
                    type="number"
                    min={1}
                  ...
                  <input
                    ...
                    className="border rounded-lg px-2 py-1 flex-1 min-w-[140px] text-sm"
                  />
          ...
          <form onSubmit={(e) => createItem(c.id, e)} className="border-t pt-3 flex flex-wrap gap-2 items-center">
            ...
            />
            <select
              value={itemFormFor(c.id).unitType}
              onChange={(e) =>
                setItemForms((prev) => ({ ...prev, [c.id]: { ...itemFormFor(c.id), unitType: e.target.value as "decimo" | "sabana" } }))
              }
