...
export default function Lottery() {
  ...
  const [historyPaymentFilter, setHistoryPaymentFilter] = useState<"all" | "paid" | "pending">("all");
  ...
  async function loadHistory() {
    const { data } = await api.get("/lottery/sales", {
      params: {
        year: historyYear,
        ...(historyFilter === "all" ? {} : { unitType: historyFilter }),
        ...(historyPaymentFilter === "all" ? {} : { paymentStatus: historyPaymentFilter }),
      },
    });
    setHistory(data);
  }
  ...
  }, [showHistory, historyFilter, historyPaymentFilter, historyYear]);
  ...
  async function saveEditItem(id: string) {
    try {
      await api.put(`/lottery/items/${id}`, {
        number: editItemForm.number,
        price: Number(editItemForm.price),
        donationAmount: Number(editItemForm.donationAmount || 0),
        unitsReceived: editItemForm.unitsReceived,
      });
      setEditingItemId(null);
      load();
    } catch (err: any) {
      setError(err.response?.data?.error || "No se ha podido guardar el número");
    }
  }
  ...
  const totalRevenue = campaigns.reduce(
    (acc, c) =>
      acc + (c.items?.reduce((s: number, i: any) => s + Number(i.donationAmount || 0) * (i.sold || 0), 0) || 0),
    0
  );
  // Dinero sin el recargo de la asociación: lo que cuesta de verdad cada
  // décimo/sábana antes de sumarle el donativo (ej. si cuesta 23 € y el
  // recargo son 3 €, esto es 20 € × décimos vendidos).
  const totalBase = campaigns.reduce(
    (acc, c) =>
      acc +
      (c.items?.reduce((s: number, i: any) => s + (Number(i.price) - Number(i.donationAmount || 0)) * (i.sold || 0), 0) ||
        0),
    0
  );
  const totalPendingPayment = campaigns.reduce(
    (acc, c) => acc + (c.items?.reduce((s: number, i: any) => s + (i.pendingPayment || 0), 0) || 0),
    0
  );
    ...
    <div className="space-y-6">
      ...
      {campaigns.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <div className="text-2xl">🎫</div>
            <div className="text-sm text-gray-500 mt-1">Total sin recargo</div>
            <div className="text-xl font-semibold">{fmt(totalBase)}</div>
          </div>
          <div className="bg-gradient-to-br from-brand to-brand-dark rounded-2xl shadow-sm p-4 text-white">
            <div className="text-2xl">💰</div>
            <div className="text-sm text-white/70 mt-1">Recaudado (recargo)</div>
      ...
      {totalPendingPayment > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center justify-between flex-wrap gap-2">
          <span className="text-sm text-amber-800">
            ⚠️ Hay <strong>{totalPendingPayment}</strong> décimos/sábanas entregados pero aún pendientes de pago
            (no cuentan como vendidos en las estadísticas de arriba).
          </span>
          <button
            onClick={() => {
              setShowHistory(true);
              setHistoryFilter("all");
              setHistoryPaymentFilter("pending");
            }}
            className="text-xs bg-amber-600 text-white px-3 py-1.5 rounded-lg shrink-0"
          >
            Ver pendientes
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 space-y-3">
          <div className="flex gap-2 flex-wrap items-center">
            ...
            {(["all", "paid", "pending"] as const).map((f) => (
              <button
                key={f}
                onClick={() => setHistoryPaymentFilter(f)}
                className={`text-xs px-3 py-1.5 rounded-lg ${
                  historyPaymentFilter === f
                    ? f === "pending"
              ...
              >
                {f === "all" ? "Cualquier estado" : f === "paid" ? "Pagados" : "Pendientes de pago"}
              </button>
            ))}
          </div>
                    ...
                    <li key={item.id} className="py-3 text-sm space-y-2">
                      {editingItemId === item.id ? (
                        <div className="bg-gray-50 rounded-lg p-2 space-y-1.5">
                          <div className="flex flex-wrap gap-2">
                            <input
                              value={editItemForm.number}
                              onChange={(e) => setEditItemForm({ ...editItemForm, number: e.target.value })}
                              placeholder="Número"
                              className="border rounded-lg px-2 py-1 text-sm w-24"
                            />
                            <input
                            ...
                            <input
                              type="number"
                              value={editItemForm.donationAmount}
                              onChange={(e) => setEditItemForm({ ...editItemForm, donationAmount: e.target.value })}
                              placeholder="Donativo/recargo €"
                              className="border rounded-lg px-2 py-1 text-sm w-32"
                            />
                            <input
                              type="number"
                              value={editItemForm.unitsReceived}
                              onChange={(e) => setEditItemForm({ ...editItemForm, unitsReceived: e.target.value })}
                              placeholder="Unidades recibidas"
                              className="border rounded-lg px-2 py-1 text-sm w-36"
                            />
                          </div>
                          <p className="text-xs text-gray-400">
                            El "Precio" debe ser el importe completo que se cobra (ej. 23 €); el "Donativo" es solo el
                            recargo (ej. 3 €) — de ahí se calcula el total "sin recargo".
                          </p>
                          <div className="flex gap-2">
                            <button onClick={() => saveEditItem(item.id)} className="text-xs bg-brand text-white px-3 py-1 rounded-lg">
                              Guardar
                            </button>
                            <button onClick={() => setEditingItemId(null)} className="text-xs text-gray-500">
                              Cancelar
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="flex justify-between items-center flex-wrap gap-1">
                          <span className="font-medium">
                            Número {item.number} — {Number(item.price).toFixed(2)} €
                            {Number(item.donationAmount) > 0 && (
                              <span className="text-xs font-normal text-gray-400"> (recargo {Number(item.donationAmount).toFixed(2)} €)</span>
                            )}
                            <button onClick={() => startEditItem(item)} className="text-xs text-brand hover:underline ml-2 font-normal">
                              Editar
                            </button>
                          </span>
