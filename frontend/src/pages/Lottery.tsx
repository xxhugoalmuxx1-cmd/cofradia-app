...
export default function Lottery() {
  ...
  async function deleteCampaign(id: string, name: string) {
    if (!window.confirm(`¿Seguro que quieres borrar la campaña "${name}"? Se borrarán también sus números y ventas. No se puede deshacer.`)) return;
    try {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {showCampaignForm && (
        <form onSubmit={createCampaign} className="bg-white rounded-2xl shadow-sm p-5 space-y-2">
      ...
      <div className="bg-white rounded-2xl shadow-sm divide-y">
        {campaigns.map((c) => (
          <div key={c.id}>
            <button onClick={() => toggleCampaign(c.id)} className="w-full flex justify-between items-center px-5 py-3 text-left">
