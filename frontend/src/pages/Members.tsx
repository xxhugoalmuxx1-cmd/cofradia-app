...
const EMPTY = { firstName: "", lastName: "", street: "", houseNumber: "", phone: "" };
...
export default function Members() {
  ...
  // Al añadir un socio nuevo a una calle/casa que ya tiene gente (misma
  // familia), se pre-rellena la calle y la casa para no tener que
  // volver a escribirlas.
  function addToHousehold(street: string, houseNumber: string) {
    setEditingId(null);
    setForm({ ...EMPTY, street, houseNumber });
    setShowForm(true);
  }
  ...
  // Agrupa por calle y, dentro de cada calle, por número de casa — así se
  // ve claramente qué socios viven juntos (misma familia) sin mezclarlos
  ...
  function groupByHouse(list: any[]) {
    const byHouse: Record<string, any[]> = {};
    for (const m of list) {
      const key = m.houseNumber || "__sin_casa__";
  ...
  return (
    <div className="space-y-6">
      ...
      <input
        placeholder="Buscar por nombre, apellidos, calle o familia…"
        value={search}
        onChange={(e) => setSearch(e.target.value)}
        className="w-full border rounded-lg px-3 py-2"
      />

      {/* Listado normal en pantalla, desplegable por calle, subagrupado por casa */}
      <div className="bg-white rounded-xl shadow-sm p-4 space-y-2">
        {Object.entries(grouped).map(([street, list]) => {
          const byHouse = groupByHouse(list);
          const houseKeys = Object.keys(byHouse);
              ...
              {openStreets[street] && (
                <div className="space-y-3 mt-1">
                  {houseKeys.map((houseKey) => {
                    const houseList = byHouse[houseKey];
                    const isHouse = houseKey !== "__sin_casa__";
                    return (
                      <div
                        key={houseKey}
                        className={isHouse ? "border border-gray-200 rounded-lg overflow-hidden" : ""}
                      >
                        {isHouse && (
                          <div className="bg-gray-50 px-3 py-1.5 flex items-center justify-between">
                            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                              👨‍👩‍👧‍👦 Familia {houseKey} <span className="font-normal text-gray-400">({houseList.length} miembro{houseList.length !== 1 ? "s" : ""})</span>
                            </span>
                            <button
                              onClick={() => addToHousehold(street, houseKey)}
                              className="text-xs text-brand hover:underline"
                            >
                              + Añadir a esta familia
                            </button>
                          </div>
                        )}
                        <ul className="divide-y">
                          {houseList.map((m) => (
                            <li key={m.id} className="py-2 px-1 text-sm flex justify-between items-center gap-2">
                              <span>
      ...
      {showForm && (
        <form onSubmit={submit} className="bg-white rounded-xl shadow-sm p-4 space-y-2 max-w-md">
          ...
          <p className="text-xs text-gray-400 -mt-1">
            Rellena "Nº familia" si conviven varios socios de una misma familia, para agruparlos juntos dentro de la calle.
          </p>
