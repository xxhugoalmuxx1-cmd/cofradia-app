...
export default function Finance() {
  ...
  async function load() {
    const { data } = await api.get(`/${tab}`, { params: { from: from || undefined, to: to || undefined } });
    setItems(data);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        <h2 className="text-sm text-gray-500 uppercase tracking-wide mb-2">Filtrar por fecha</h2>
        <div className="flex flex-wrap gap-2 items-end">
          ...
          <button onClick={load} className="bg-gray-700 text-white rounded-lg px-4 py-2 text-sm">Filtrar</button>
          {(from || to) && (
            <button
