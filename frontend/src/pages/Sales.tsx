  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-40 md:mb-32">
        ...
        {products.length === 0 && (
          <p className="text-gray-400 text-sm col-span-full">Aún no hay productos. Créalos en la sección "Productos".</p>
        )}
      </div>
          ...
          {Object.entries(cart).map(([id, line]) => {
            ...
            return (
              <li key={id} className="py-2 flex flex-wrap items-center gap-2 text-sm">
                <span className="flex-1 min-w-[100px] truncate">{product.name}</span>
                <button onClick={() => removeFromCart(id)} className="px-2 border rounded">−</button>
                <span className="w-6 text-center">{line.quantity}</span>
                <button onClick={() => addToCart(product)} className="px-2 border rounded">+</button>
                <span className="text-xs text-gray-400">precio unidad</span>
                <input
                  type="number"
                  step="0.01"
                  value={line.price}
