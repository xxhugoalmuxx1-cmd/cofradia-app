...
export default function Fees() {
  ...
  async function deletePeriod(id: string, name: string) {
    if (!(await confirm({ title: "Borrar periodo", message: `¿Borrar el periodo "${name}"? Se borrarán también todas sus cuotas. No se puede deshacer.`, danger: true, confirmLabel: "Borrar" }))) return;
  ...
  async function removeFee(feeId: string) {
    if (!(await confirm({ title: "Borrar cuota", message: "¿Borrar esta cuota? No se puede deshacer.", danger: true, confirmLabel: "Borrar" }))) return;
      ...
      {selectedPeriod && (
        <div className="bg-white rounded-xl shadow-sm p-4 space-y-3">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <h2 className="font-medium">Cuotas del periodo</h2>
            <input
          ...
          {Object.entries(
            fees
              .filter((f) => {
                ...
                return (
                  f.member?.firstName?.toLowerCase().includes(q) ||
                  f.member?.lastName?.toLowerCase().includes(q) ||
                  f.member?.street?.toLowerCase().includes(q)
                );
              })
              .sort((a, b) => (a.member?.street || "").localeCompare(b.member?.street || "") || (a.member?.lastName || "").localeCompare(b.member?.lastName || ""))
              .reduce((acc: Record<string, any[]>, f) => {
                const key = f.member?.street || "Sin calle";
                (acc[key] = acc[key] || []).push(f);
                return acc;
              }, {})
          ).map(([street, list]) => (
            <div key={street}>
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">{street}</h3>
              <ul className="divide-y">
                {list.map((f) => (
