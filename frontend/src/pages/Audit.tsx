...
export default function Audit() {
  ...
  async function clearOlderThan() {
    const date = window.prompt("Borrar todo lo anterior a esta fecha (AAAA-MM-DD):");
    if (!date) return;
    if (!window.confirm(`Esto borrará todos los registros anteriores a ${date}. ¿Continuar?`)) return;
    try {
      const { data } = await api.delete("/audit-logs", { params: { before: date } });
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <button onClick={clearOlderThan} className="text-xs text-red-600 hover:underline">
