    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        if (item.productId) {
          ...
          labels.push(item.quantity > 1 ? `${product.name} x${item.quantity}` : product.name);
        } else {
          ...
          labels.push(item.quantity > 1 ? `${item.description} x${item.quantity}` : item.description);
        }
      }

      // Concepto legible con lo que se ha vendido (en vez del ID interno de
      // la venta, que no decía nada al mirarlo en Caja o Banco).
      let saleConcept = `Venta rápida: ${labels.join(", ")}`;
      if (saleConcept.length > 200) saleConcept = saleConcept.slice(0, 197) + "…";
        ...
        if (account) {
          ...
          await tx.bankMovement.create({
            data: {
              ...
              type: "in",
              amount: total,
              concept: `${saleConcept} (${paymentMethod})`,
              resultingBalance: newBalance,
            },
          });
        }
