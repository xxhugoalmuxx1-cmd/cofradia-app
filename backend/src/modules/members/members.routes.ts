...
router.get("/", requirePermission("members.read"), async (req, res) => {
  ...
  const allSorted = await prisma.member.findMany({
    select: { id: true },
    orderBy: [{ street: "asc" }, { houseNumber: "asc" }, { lastName: "asc" }],
  });
      ...
      ...(search
        ? {
            OR: [
              ...
              { houseNumber: { contains: String(search), mode: "insensitive" } },
            ],
          }
        : {}),
    },
...
router.get("/export", requirePermission("members.read"), async (_req, res) => {
  ...
  for (const [street, list] of grouped) {
    ...
    // Dentro de cada calle, se subdivide por número de casa (para
    // distinguir familias que viven juntas de socios sueltos).
    const byHouse = new Map<string, typeof list>();
    for (const m of list) {
      const key = m.houseNumber || "";
      if (!byHouse.has(key)) byHouse.set(key, []);
      byHouse.get(key)!.push(m);
    }

    let rowIndex = 0;
    for (const [house, houseList] of byHouse) {
      if (house) {
        ...
        doc.font("Helvetica-Bold").fontSize(8).fillColor("#6b1530").text(`Familia ${house} (${houseList.length})`, doc.page.margins.left + 4, y + 4);
