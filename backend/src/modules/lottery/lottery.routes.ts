  ...
  try {
    ...
    const withSummary = campaigns.map((c) => ({
      ...
      items: c.items.map((item) => {
        ...
        // Lo pendiente de pago NO cuenta como vendido de verdad todavía.
        const sold = item.sales
          .filter((s) => s.paymentStatus === "paid")
          .reduce((acc, s) => acc + (s.unitType === "sabana" ? s.unitsSold * 10 : s.unitsSold), 0);
        const pendingPayment = item.sales
          .filter((s) => s.paymentStatus === "pending")
          .reduce((acc, s) => acc + (s.unitType === "sabana" ? s.unitsSold * 10 : s.unitsSold), 0);
        const pending = item.unitsReceived != null ? item.unitsReceived - sold : null;
        return { ...item, sold, pending, pendingPayment, sales: undefined };
      }),
    }));
...
// Editar un número ya creado — por ejemplo, para corregir el precio o el
// recargo si se metieron mal al darlo de alta (el "Precio" debe ser
// siempre el importe completo que se cobra; "Donativo" es solo el recargo
// que se resta de ahí para calcular el total "sin recargo").
router.put("/items/:id", requirePermission("lottery.update"), async (req: AuthedRequest, res, next) => {
  try {
    const { number, price, donationAmount, unitsReceived } = req.body;
    const item = await prisma.lotteryItem.update({
      where: { id: req.params.id },
      data: {
        number: number !== undefined ? String(number) : undefined,
        price: price !== undefined ? Number(price) : undefined,
        donationAmount: donationAmount !== undefined ? Number(donationAmount) : undefined,
        unitsReceived:
          unitsReceived === "" || unitsReceived === null
            ? null
    ...
    await logAudit({ userId: req.user!.id, action: "update", module: "lottery_items", recordId: item.id, newValue: req.body });
    res.json(item);
  } catch (err) {
    next(err);
  }
...
router.get("/sales", requirePermission("lottery.read"), async (req, res, next) => {
  try {
    const { campaignId, unitType, year, paymentStatus } = req.query;
