...
import { requirePermission, requireAdmin } from "../../middlewares/permissions";
...
// Movimientos filtrables por mes/año (para poder revisar meses anteriores,
// no solo los últimos 50 movimientos que devuelve el endpoint de arriba).
router.get("/:id/movements", requirePermission("cash.read"), async (req, res) => {
  ...
  const movements = await prisma.cashMovement.findMany({
    where: { cashRegisterId: req.params.id, createdAt: gte ? { gte, lte } : undefined },
...
router.post("/:id/movements", requireAdmin, async (req: AuthedRequest, res, next) => {
  try {
...
router.post("/:id/withdraw", requireAdmin, async (req: AuthedRequest, res, next) => {
  try {
...
// Cierre de caja
router.post("/:id/close", requireAdmin, async (req: AuthedRequest, res, next) => {
  try {
