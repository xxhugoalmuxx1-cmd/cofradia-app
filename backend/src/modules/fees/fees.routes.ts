...
router.put("/periods/:id", requirePermission("fees.create"), async (req: AuthedRequest, res, next) => {
  try {
    const { name, amount, year } = req.body;
    const period = await prisma.feePeriod.update({ where: { id: req.params.id }, data: { name, amount, year } });
    await logAudit({ userId: req.user!.id, action: "update", module: "fee_periods", recordId: period.id });
    res.json(period);
  } catch (err) {
    next(err);
  }
...
// Borra el periodo y, en cascada, todas las cuotas individuales generadas
// para ese periodo (configurado en el esquema con onDelete: Cascade).
router.delete("/periods/:id", requirePermission("fees.create"), async (req: AuthedRequest, res, next) => {
  try {
    await prisma.feePeriod.delete({ where: { id: req.params.id } });
    await logAudit({ userId: req.user!.id, action: "delete", module: "fee_periods", recordId: req.params.id });
    res.json({ ok: true });
  } catch (err) {
    next(err);
  }
